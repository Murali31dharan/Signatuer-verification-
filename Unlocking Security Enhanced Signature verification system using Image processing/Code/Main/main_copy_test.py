...
def compare_lbp_signatures(signature1, signature2):
    ...
    threshold = 0.8
    similarity = chi_squared_distance < threshold
    # Plot LBP_Histogram
...
def show_graph(path1, path2):
    try:
        ...
        plt.colorbar()
        plt.title('Heatmap - Pixel-wise Abs. Difference')
        plt.xlabel('Pixel Column')
    ...
    while True:
        ...
        if k % 256 == 27:
            ...
            if cv2.imwrite(filename=img_name, img=frame):
                ...
                provide_feedback("Image captured successfully.", "green")
                break
            else:
                print(f"Failed to write {img_name}")
                provide_feedback("Error: Failed to capture image.", "red")
            # img_counter += 1
    cam.release()
...
def checkSimilarity(window, path1, path2):
    ...
    if(result <= thresholds):
        ...
        accuracy = (1 - (result / 100)) * 100
        messagebox.showinfo("Accuracy Rate", f"Accuracy Rate: {accuracy}%")
        pass
    else:
...
def display_elements():
    ...
    img1_message = tk.Label(root, text="Signature 1", font=10, bg="#E7E8D1")
    ...
    img2_message = tk.Label(root, text="Signature 2", font=10, bg="#E7E8D1")
